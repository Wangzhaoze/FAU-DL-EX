...
class LSTM(BaseLayer):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.FC2 = FullyConnected(self.input_size + self.hidden_size, self.hidden_size)
        self.Sigmoid2 = Sigmoid()

        self.FC3 = FullyConnected(self.input_size + self.hidden_size, self.hidden_size)
        self.TanH3 = TanH()

        self.FC4 = FullyConnected(self.input_size + self.hidden_size, self.hidden_size)
    ...
    def forward(self, input_tensor):
        ...
        self.c_archive = np.zeros((batch_size + 1, self.hidden_size))

        self.o_archive = np.zeros((batch_size, self.hidden_size))
        self.u_archive = np.zeros((batch_size, self.hidden_size))
        self.f_archive = np.zeros((batch_size, self.hidden_size))
        self.i_archive = np.zeros((batch_size, self.hidden_size))
        self.c_wave_archive = np.zeros((batch_size, self.hidden_size))

        for t in range(batch_size):
            x = input_tensor[t, :]

            # hx2u_input is a combination vector of x and h
            hx_input = np.append(x, self.hidden_state).reshape((1, -1))


            # calculate u, h, o according to the formal
            f = self.Sigmoid1.forward(self.FC1.forward(hx_input))
            i = self.Sigmoid2.forward(self.FC2.forward(hx_input))
            c_wave = self.TanH3.forward(self.FC3.forward(hx_input))
            o = self.Sigmoid4.forward((self.FC4.forward(hx_input)))
            c = self.cell_state * f + c_wave * i
            h = self.TanH5.forward(c) * o

            # save computation results
            self.hidden_state = list(h.T)
            self.h_archive[t + 1] = list(h.T)
            self.cell_state = list(c.T)
            self.c_archive[t + 1] = list(c.T)
            self.f_archive[t] = list(f.T)
            self.i_archive[t] = list(i.T)
            self.c_wave_archive[t] = list(c_wave.T)
            self.o_archive[t] = list(o.T)
            output[t] = self.Sigmoid.forward(self.h2o.forward(h))

        # if we wouldn't memorize current h, reset
